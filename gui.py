...
class MainDialog(wx.Dialog):
    ...
    def on_get_chars_done(self, query, chars, start):
        if query == self.query:
            ...
            for char in chars:
                ...
                if i == sel:
                    self.char = char
                    self.char_lbl.SetLabel(unichr(char['id']))
                i += 1
            if dirty:
    ...
    def on_item_selected(self, event): # wxGlade: MainDialog.<event_handler>
        ...
        try:
            ...
            if char != None:
                self.char = char
                self.char_lbl.SetLabel(unichr(self.char['id']))
        except KeyError:
            pass
        event.Skip()
    ...
    def on_candidate_resize(self, event):
        width = self.candidate_lst.GetClientSizeTuple()[0]
...
class CharList(wx.ListCtrl):
    ...
    def OnGetItemText(self, item, col):
        try:
            ...
            return ''
        return (unichr(char['id']), char['name'].capitalize())[col]
